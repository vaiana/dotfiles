...
- `~/.gnupg/` — GPG keyring. Needed because `~/.gitconfig` sets `commit.gpgsign = true` with a specific `signingkey`. Without it, commits will fail to sign.
- `~/.gitconfig` — git user/email/signing key/editor config. Small and safe to copy as-is.
